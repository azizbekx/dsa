...
public class HeapMax {
    private void maxHeapify(int[] heapArray, int index, int heapSize) {
        ...
        while (largest < heapSize / 2) { // check parent nodes only
            ...
            if (left < heapSize && heapArray[left] > heapArray[index]) largest = left;

            if (right < heapSize && heapArray[right] > heapArray[largest]) largest = right;
    ...
    public static void main(String[] args) {
        ...
        new HeapMax().buildMaxHeap(heapArray, heapArray.length);
